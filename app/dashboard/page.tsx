...
export default function Dashboard() {
  ...
  const fetchAppointments = async (token:string) => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://elitebarber.atwebpages.com/php-backend/api'}/appointments`, { headers: { 'Authorization':`Bearer ${token}` }});
  ...
  const cancelAppointment = async (id:number) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://elitebarber.atwebpages.com/php-backend/api'}/appointments/${id}`, { method:'DELETE', headers:{'Authorization':`Bearer ${token}`}});
      const data = await res.json();
