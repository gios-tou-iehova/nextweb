...
export default function Dashboard() {
  ...
  const fetchAppointments = async (token:string) => {
    try {
      const res = await fetch('http://localhost/backend/api/appointments', { headers: { 'Authorization':`Bearer ${token}` }});
  ...
  const cancelAppointment = async (id:number) => {
    ...
    try {
      const res = await fetch(`http://localhost/backend/api/appointments/${id}`, { method:'DELETE', headers:{'Authorization':`Bearer ${token}`}});
      const data = await res.json();
