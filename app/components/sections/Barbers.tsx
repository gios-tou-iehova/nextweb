...
const Barbers = () => {
  ...
  useEffect(() => { fetch(`${process.env.NEXT_PUBLIC_API_URL || '/api/proxy'}/barbers`).then(r=>r.json()).then(d=>{ if(d.status==='success') setBarbers(d.data); }).catch(()=>{}).finally(()=>setLoading(false)); }, []);
